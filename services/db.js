...
var fs = require('fs');
var mongo = require('mongoskin'); // mongoskin for connecting and querying
									// mongodb
...
exports.getrashdriving = function calcrashdriving(req, res) {
	...
	db.collection("rashdriving").find({
		a : {
			$gt : 250
		}
	}).toArray(function(err, items) {
...
exports.calcrashdriving = function calcrashdriving(req, res) {
	...
	db.collection("potholes").find({
		th : 200
	}).toArray(function(err, allpotholesabove200) {
		db.close();
		...
		groups.push(group);
		var finalRes = [];
		for (var i = 1; i < allpotholesabove200.length; i++) {
			var diff = allpotholesabove200[i].t - prev.t;
			if (diff < 1000)
				group.push(allpotholesabove200[i]);
			else {
				if (group.length == 1)
					groups.pop();
				else {
					finalRes.push(group[0]);
					console.log(group.length);
				}
				var newgrp = [ allpotholesabove200[i] ];
				groups.push(newgrp);
				group = newgrp;
			}
...
exports.calcall = function calcall() {
	...
	sens.forEach(function(s) {
		console.log("Calculating for Sensitivity = " + s);
		calc(s);
	});
};
...
function calc(th) {
	...
	db.collection("data").aggregate([ {
		...
		items.forEach(function(d) {
			...
			}).toArray(function(err, it) {
				console.log("Second Query");
				i++;
				if (it != undefined && it[0] != undefined) {
					d.x = it[0].d2;
					d.y = it[0].d3;
					// dist = 6400000 * 2 * Math.PI * Math.sqrt((d.x -
					// 12.960295563367373)*(d.x - 12.960295563367373) + (d.y -
					// 77.52550212841176)*(d.y - 77.52550212841176)) / 360.0; //
					// dist in metres
...
exports.getaccpoints = function getaccpoints(req, res) {
	...
	db.collection('data').aggregate([ {
		...
		items.forEach(function(it) {
			it.x = (it.x - xmin) / 1000.00;
			it.y = Math.round(it.y, 0);
		});
		res.end(JSON.stringify(items));
		db.close();
	});
...
function getConnection() {
	var db = mongo.db(c.dbserver + "://" + c.host + ":" + c.port + "/" + c.db,
			{
				native_parser : true
			});
	return db;
